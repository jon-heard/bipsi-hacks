...
//!CONFIG tweak--new-plugins-have-filename-tag (text) "false"
//!CONFIG tweak--new-fields-default-to-tag-type (text) "true"
//!CONFIG tweak--tilset-export-ordered-by-tile (text) "false"
...
function setupSaveButtonShowsWhenSaved() {
	...
	const refreshSavedIndicator = () => {
		...
		if (stateManager.index == savedHistoryIndex) {
	...
	const onSave = () => {
		const stateManager = window.EDITOR.stateManager;
		savedHistoryIndex = stateManager.index;
	...
	if (EDITOR.stateManager.history.length === 1) {
		onSave();
	}
...
function setupEditingHotkeys() {
	...
	document.getElementById('tile-map-paint').addEventListener('click', () => {
		if (EDITOR.roomPaintTool.value == 'events') {
	...
	document.getElementById('tile-select').addEventListener('click', () => {
		if (EDITOR.roomPaintTool.value == 'tile') {
			currentFocus = FOCUS.TILE;
		}
	});
	
	function copyTile() {
		const { tile, tileset } = EDITOR.getSelections();
	...
	function pasteTile() {
		// Modified from 'EDITOR.duplicateTile()'
        EDITOR.stateManager.makeChange(async (data) => {
            const { tileIndex, tileset } = EDITOR.getSelections(data);
	...
	document.body.addEventListener('keydown', e => {
		if (currentFocus) {
			if (e.code == 'Delete') {
				try {
					switch (currentFocus) {
						case FOCUS.EVENT:
							...
							break;
						case FOCUS.FIELD:
							EDITOR.eventEditor.removeField();
							break;
						case FOCUS.TILE:
							EDITOR.deleteTile();
							break;
				...
				try {
					switch (currentFocus) {
						...
						case FOCUS.FIELD:
							...
							clipboardForFields = COPY(field);
							// Disable other copy types
							EDITOR.actions.pasteEvent.disabled = true;
							clipboardForTilesFrameCount = 0;
							break;
						case FOCUS.TILE:
							copyTile();
				...
				try {
					switch (currentFocus) {
						case FOCUS.EVENT:
							if (EDITOR.actions.pasteEvent.disabled) return; // Don't paste if paste button isn't clickable
							EDITOR.pasteSelectedEvent();
							break;
						case FOCUS.FIELD:
							if (!clipboardForFields) return;
							const eventEditor = EDITOR.eventEditor;
							// Modified from "EventEditor.addField()" as we also need to set the new field's values
							EDITOR.stateManager.makeChange(async (data) => {
								const { event, fieldIndex } = eventEditor.getSelections(data);
					...
					switch (currentFocus) {
						case FOCUS.EVENT: //TODO
							...
							if (newPosition[0] !== event.position[0] || newPosition[1] !== event.position[1]) {
								EDITOR.stateManager.makeChange(async (data) => {
									const eventToModify = findEventById(data, event.id);
									eventToModify.position[0] = newPosition[0];
									eventToModify.position[1] = newPosition[1];
									EDITOR.selectedEventCell = { x: newPosition[0], y: newPosition[1] };
								});
							}
							break;
						case FOCUS.FIELD:
							EDITOR.eventEditor.shiftField((d[0] == -1 || d[1] == -1) ? -1 : 1);
							break;
						case FOCUS.TILE:
							const data = EDITOR.stateManager.present;
							const { tileIndex: startIndex } = EDITOR.getSelections(data);
							const endIndex = Math.max(0, Math.min(data.tiles.length - 1, startIndex + d[0] + d[1] * 9));
							if (endIndex !== startIndex) {
								EDITOR.stateManager.makeChange(async (data) => {
									[data.tiles[endIndex], data.tiles[startIndex]] = [data.tiles[startIndex], data.tiles[endIndex]];
									EDITOR.tileBrowser.select.setValueSilent(endIndex);
									EDITOR.tileBrowser.select.inputs[endIndex].focus();
								});
							}
		...
		if (e.code =='KeyZ' && e.ctrlKey) {
			...
			if (e.shiftKey) {
				EDITOR.actions.redo.invoke();
			} else {
				EDITOR.actions.undo.invoke();
			}
