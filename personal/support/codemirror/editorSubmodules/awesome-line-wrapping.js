...
let line_wrapping_decorations = StateField.define({
    ...
    update(deco, tr) {
        ...
        for (let i of [...Array(tr.state.doc.lines).keys()]) {
            ...
            rules.setProperty("text-indent", "calc(-1 * var(--idented) - 1px - 1.5rem)") // I have no idea why, but without the - 1px it behaves weirdly periodically
            rules.setProperty("padding-left", "calc(var(--idented) + var(--cm-left-padding, 4px) + 1.5rem)")
